...
if (designMobel) {
  designMobel.addEventListener("submit", async (e) => {
    ...
    const valgtMobel = designMobel.querySelector(
      'input[name="furniture_type"]:checked'
    )?.value;
    const valgtTrae = designMobel.querySelector(
      'input[name="tree_type"]:checked'
    )?.value;

    const andetFelt = designMobel
      .querySelector('input[name="mobel_andet"]')
      ?.value.trim();
    ...
    const orderData = {
      ...
      custom_postal:
        parseInt(document.getElementById("custom_postal").value) || 0,
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        await emailjs
          .send("service_snedker", "template_w2ok64w", adminParametre)
          .then(() => console.log("Email sendt til admin"))
          .catch((error) =>
            console.error(
              "Der skete en fejl ved sending af email til admin:",
              error
            )
          );

        window.location.href = `bekraeftelse.html?ordre=DESIGN`;
      }
    } catch (error) {
      console.error("Der skete en fejl:", error.message);
      alert(
        "Kunne ikke sende din bestilling. Prøv igen eller kontakt mig direkte."
      );
    }
  });
}