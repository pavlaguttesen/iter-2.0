...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function gemValgtTid(datoStreng) {
    ...
    const pænDato = dato.toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long' });
    const pænTid = dato.toLocaleTimeString('da-DK', { hour: '2-digit', minute: '2-digit' });

    document.querySelector('#modal-valgt-tid').innerText = `Valgt tid: ${pænDato} kl. ${pænTid}`;
...
function setupModalEvents(calendar) {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            e.preventDefault();
            const navn = document.querySelector('#kunde-navn').value;
